...
def evaluate_mdp_policy(mdp_problem, mdp_policy):
    ...
    def evaluate_once(mdp_problem, mdp_policy):
        ...
        for j in range(num_steps_per_trial):
            ...
            # print("mdp_policy[0]", mdp_policy[0])
            #print(type(mdp_policy))
...
def evaluate_mdp_policy_sensitivity(mdp_problem, mdp_policy, seed=42):

    # Add seed to keep consistency
    random.seed(seed)
    np.random.seed(seed)
    ...
    std_deviation = np.std(rewards, ddof=1)

    sem = std_deviation / np.sqrt(num_trials) # standard error of the mean

    return average_reward, sem